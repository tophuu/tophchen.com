...
export default function RootLayout({
  ...
  const bootstrapScript = `(function(){var b=document.body,V=['about','projects','contact'];try{var S='pref-session-ts',M=86400000,now=Date.now(),raw=localStorage.getItem(S),last=raw?Number(raw):0;if(last>0&&now-last>M){for(var i=localStorage.length-1;i>=0;i--){var k=localStorage.key(i);if(k&&k.indexOf('pref-')===0)localStorage.removeItem(k)}}localStorage.setItem(S,String(now));var w=localStorage.getItem('pref-wallpaper');b.dataset.wallpaper=w||'big-sur';var t=localStorage.getItem('pref-theme');b.dataset.notesTheme=(t==='black'||t==='white')?t:'white';var n=localStorage.getItem('pref-active-note');var a=(n&&V.indexOf(n)>=0)?n:'about';b.dataset.activeNote=a;if(a!=='about'){b.classList.add('note-booting');setTimeout(function(){b.classList.remove('note-booting')},1500)}}catch(e){b.dataset.wallpaper='big-sur';b.dataset.notesTheme='white';b.dataset.activeNote='about'}b.classList.add('no-transition')})()`;
  return (
    <html lang="en">
